...
import { textValue, iconValue } from "@/lib/templates/content";
...
export function Navigation({
  ...
  return <header className="nav" data-screen-label="00 Nav">
      <div className="wrap nav-inner">
        ...
        <nav className="nav-links" aria-label="Primary">
          {navItems.map(item => <a key={item.key} href={item.href}>{t(item.key)}</a>)}
        </nav>
        <div className="nav-cta">
          ...
          <a href="#book" className="btn">{"Request Demo"}</a>
        </div>
        <button type="button" className="nav-burger" aria-label={menuOpen ? "Close menu" : "Open menu"} aria-expanded={menuOpen} aria-controls="brightway-mobile-menu" onClick={() => setMenuOpen(value => !value)}>
          <span aria-hidden="true" />
          <span aria-hidden="true" />
        </button>
      </div>
      <div id="brightway-mobile-menu" className={`mobile-menu${menuOpen ? " open" : ""}`}>
        <div className="mobile-menu-inner">
          {navItems.map(item => <a key={item.key} href={item.href} onClick={() => setMenuOpen(false)}>
              {t(item.key)}
            </a>)}
          <a href="#" onClick={() => setMenuOpen(false)}>{"Sign In"}</a>
          <a href="#book" className="btn" onClick={() => setMenuOpen(false)}>{"Request Demo"}</a>
        </div>
      </div>
    </header>;
}